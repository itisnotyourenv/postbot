...
async def choose_type(
    ...
    logger.info("User %s chose post type: %s", callback.from_user.id, callback_data.content_type)
    ...
    await callback.message.edit_text(text=i18n.get(prompt_key), reply_markup=back_markup(i18n, MainMenuCBData.create_post))
    await callback.answer()
...
async def collect_buttons(
    ...
    await _show_preview(message, state, i18n, edit=False)
...
async def _show_preview(
    ...
    *,
    ...
    # For media, we try to send the media for preview
